...
template sudoku() {
    ...
    component rangeProofPuzzle = RangeProof(4);
    rangeProofPuzzle.range[0] <== 0;
    rangeProofPuzzle.range[1] <== 9;

    component rangeProofSolution = RangeProof(4);
    rangeProofSolution.range[0] <== 0;
    rangeProofSolution.range[1] <== 9;

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProofPuzzle.range[j] <== puzzle[i][j];
            rangeProofPuzzle.out <== 1;

            rangeProofSolution.range[j] <== solution[i][j];
            rangeProofSolution.out <== 1;
